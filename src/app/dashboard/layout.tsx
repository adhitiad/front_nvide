...
export default function DashboardLayout({
  ...
  // If loading session, show beautiful immersive anime skeleton loader
  if (isPending || (!session && isPending)) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center text-foreground space-y-4">
        ...
        <p className="text-sm text-muted-foreground font-semibold tracking-wide anime-pulse-hover">
          Connecting to NVide Live secure channel...
        </p>
      </div>
    );
  }
  ...
  const isAdminRoute = pathname?.includes("/dashboard/moderation") || pathname?.startsWith("/admin");
      ...
      <div className="min-h-screen h-full relative bg-background text-foreground flex flex-col md:flex-row">
        ...
        <main className="md:pl-72 flex flex-col flex-1 min-h-screen pb-16 md:pb-0">
          ...
          <div className="flex-1 p-8 flex flex-col items-center justify-center text-center max-w-xl mx-auto space-y-6">
            <div className="h-20 w-20 bg-destructive/10 border border-destructive/30 rounded-3xl flex items-center justify-center text-destructive shadow-xl shadow-destructive/5 animate-bounce">
              <ShieldAlert className="h-10 w-10" />
            </div>
            <div className="space-y-2">
              <h2 className="text-2xl font-black text-foreground tracking-tight">Access Denied! / Akses Ditolak!</h2>
              <p className="text-muted-foreground text-sm">
                Maaf, Halaman Administrasi ini hanya dapat diakses oleh Administrator dengan kredensial tingkat tinggi.
              </p>
            </div>
            <Button 
            ...
            >
              Back to Main Dashboard / Kembali
            </Button>
          </div>
        </main>
        <BottomNav />
  ...
  return (
    <div className="min-h-screen h-full relative bg-background text-foreground flex flex-col md:flex-row">
      ...
      <Dialog open={mobileSidebarOpen} onOpenChange={setMobileSidebarOpen}>
        <DialogContent className="w-screen h-screen max-w-none rounded-none p-0 md:hidden">
          <Sidebar />
        </DialogContent>
      </Dialog>
    </div>
