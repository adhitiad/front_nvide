      ...
      <div className="relative bg-gradient-to-b from-primary/20 to-background pt-12 pb-6 px-4">
        ...
        <div className="flex items-center gap-4 mt-4">
          <div className="relative">
            <div className="w-20 h-20 rounded-full border-2 border-primary bg-card overflow-hidden">
              <Image 
                src={session?.user?.image || "https://images.unsplash.com/photo-1542751371-adc38448a05e?q=80&w=200"} 
                alt="Profile" 
                fill 
          ...
          <div className="flex-1">
            <h1 className="text-xl font-heading font-black">{session?.user?.name || "Guest User"}</h1>
            <p className="text-xs text-muted-foreground font-semibold mb-2">ID: 88291047</p>
        ...
        <div className="flex justify-around mt-8 bg-card rounded-2xl p-4 shadow-sm border border-primary/10">
          <div className="flex flex-col items-center">
            <span className="font-heading font-black text-lg">142</span>
            <span className="text-[10px] text-muted-foreground font-bold">Following</span>
          </div>
          <div className="w-px bg-border/50" />
          <div className="flex flex-col items-center">
            <span className="font-heading font-black text-lg">8.4k</span>
            <span className="text-[10px] text-muted-foreground font-bold">Followers</span>
          </div>
          <div className="w-px bg-border/50" />
          <div className="flex flex-col items-center">
            <span className="font-heading font-black text-lg">942</span>
            <span className="text-[10px] text-muted-foreground font-bold">Visitors</span>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 space-y-4 mt-2">
        ...
        <div className="bg-card rounded-2xl p-4 flex items-center justify-between shadow-sm border border-primary/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider">My Wallet</p>
              <p className="font-heading font-black text-lg text-foreground flex items-center gap-1">
                <span className="text-yellow-500">🪙</span> 14,500
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3 bg-card rounded-2xl p-4 shadow-sm border border-primary/10">
          {[
            { icon: Crown, label: "VIP Center", color: "text-amber-500", bg: "bg-amber-500/10", href: "/vip" },
            { icon: Users, label: "My Family", color: "text-indigo-500", bg: "bg-indigo-500/10", href: "/family" },
            { icon: ShieldCheck, label: "Agency", color: "text-emerald-500", bg: "bg-emerald-500/10", href: "/agency" },
            { icon: Star, label: "Tasks", color: "text-pink-500", bg: "bg-pink-500/10", href: "/tasks" },
          ].map((item, i) => (
            <Link key={i} href={item.href} className="flex flex-col items-center gap-2 group">
              <div className={`w-12 h-12 rounded-2xl ${item.bg} flex items-center justify-center group-hover:scale-110 transition`}>
        ...
        <div className="bg-card rounded-2xl overflow-hidden shadow-sm border border-primary/10">
          {[
            ...
            <div key={i} className="flex items-center justify-between p-4 border-b border-border/50 last:border-0 active:bg-muted/50 cursor-pointer">
              <div className="flex items-center gap-3">
