...
import VIPEntryEffect, {
  VIPEntryEvent,
} from "@/components/live/VIPEntryEffect";
...
export default function StreamDetailPage({ params }: PageProps) {
  ...
  const { chatMessages, viewerCount, likes, setLikes } = useStreamStore();
  ...
  const [luckyBagLoading, setLuckyBagLoading] = useState(false);
  const [activeLuckyBagId, setActiveLuckyBagId] = useState<string | null>(null);
  ...
  // Fetch active lucky bag for this stream
  useEffect(() => {
    if (!streamId || !showLuckyBag) return;
    async function fetchLuckyBag() {
      try {
        const res: any = await api.get(`/streams/${streamId}/lucky-bags`);
        const bags = Array.isArray(res) ? res : res?.data || [];
        if (bags.length > 0) {
          setActiveLuckyBagId(bags[0].id);
        }
      } catch {
        // No lucky bag available for this stream
        setActiveLuckyBagId(null);
      }
    }
    fetchLuckyBag();
  }, [streamId, showLuckyBag]);
  ...
  // Real Lucky Bag claim via API
  const handleClaimLuckyBag = useCallback(async () => {
    if (!activeLuckyBagId) {
      return {
        success: false,
        coins: 0,
        message: "No active lucky bag available",
      };
    }
    setLuckyBagLoading(true);
    try {
      const res: any = await api.post(
        `/lucky-bags/${activeLuckyBagId}/claim`,
        {},
      );
      ...
      const msg = err?.response?.data?.message || "Failed to claim lucky bag";
      return { success: false, coins: 0, message: msg };
    } finally {
      setLuckyBagLoading(false);
    }
  }, [activeLuckyBagId]);

  // Real Wheel spin via API
  const handleSpinWheel = useCallback(async () => {
    setWheelSpinning(true);
    try {
      const res: any = await api.post("/wheel/spin", {});
      const data = res.data || res;
      return {
        prizeId: data.prize_id || data.prizeId || "empty",
        message: data.message || "Success",
      };
    } catch (err: any) {
      const msg = err?.response?.data?.message || "Failed to spin the wheel";
      toast.error(msg);
      return { prizeId: "empty", message: msg };
    } finally {
    ...
    <div className="h-screen w-full bg-black relative overflow-hidden flex flex-col font-sans">
      ...
      <div className="absolute inset-0 z-10 flex flex-col pointer-events-none">
        ...
        <div className="p-4 flex items-start justify-between pointer-events-auto">
          ...
          <div className="flex items-center gap-2 bg-black/40 backdrop-blur-md rounded-full p-1 pr-4 border border-white/10">
            ...
            <div className="relative w-8 h-8 rounded-full overflow-hidden border border-primary">
              <Image
                src={
                  currentStream.host?.avatar_url ||
                  "https://images.unsplash.com/photo-1542751371-adc38448a05e?q=80&w=100"
                }
                alt="Host"
                fill
            ...
            <div className="flex flex-col">
              <span className="text-white text-xs font-bold leading-tight">
                {currentStream.host?.username || "HostName"}
              </span>
              <span className="text-primary text-[9px] font-black uppercase tracking-wider">
                Level 42
              </span>
            </div>
            <Button
              size="sm"
              className="ml-2 h-6 px-3 bg-primary hover:bg-primary/90 text-white text-[10px] rounded-full font-bold"
            >
              Follow
            </Button>
          </div>
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-2">
              <div className="flex -space-x-2">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="w-6 h-6 rounded-full border border-white bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-[8px] text-white font-bold z-10"
                  >
                    V{i}
                  </div>
                ))}
            ...
            <div className="flex gap-2">
              <button
                ...
                className="w-8 h-8 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center border border-white/10 text-white hover:text-primary transition"
              >
                <Swords className="w-4 h-4" />
              </button>
              <button
                onClick={() => setShowLuckyBag(true)}
                className="w-8 h-8 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center border border-white/10 text-white hover:text-primary transition"
              >
        ...
        <div className="flex-1 flex items-end p-4 pb-20 pointer-events-auto w-full md:w-2/3 lg:w-1/3">
          <div
            className="w-full h-64 overflow-y-auto scrollbar-hide space-y-2 relative"
            style={{
          ...
          >
            ...
            <div className="bg-primary/20 backdrop-blur-md rounded-xl p-2 text-[11px] text-white border border-primary/30 font-semibold shadow-sm">
              ...
              Welcome to the official broadcast! Please follow the community
              guidelines.
            </div>
            ...
            {chatMessages.map((msg, i) => (
              ...
              >
                <div className="bg-black/40 backdrop-blur-md rounded-xl px-2.5 py-1.5 flex gap-1.5 items-center border border-white/5">
                  {/* VIP Badge mock */}
                  {i % 5 === 0 && (
                    <span className="bg-gradient-to-r from-amber-400 to-orange-500 text-[8px] font-black px-1 rounded-sm text-black">
                      SVIP
                    </span>
                  )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 pt-0 flex items-center gap-2 pointer-events-auto bg-gradient-to-t from-black/80 to-transparent">
          <form onSubmit={handleSendChat} className="flex-1 relative">
            ...
            <button
              type="submit"
              className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-primary flex items-center justify-center text-white"
            >
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center border border-white/10 text-white hover:bg-white/20 transition shrink-0"
          >
